...
class PathGenerator(LifecycleNode):
    """
    ...
    **Notes**:
    ...
    current_buoy_positions: dict[int, BuoyDetectionStamped] = {}
    current_buoy_times = {}
    ...
    def heading_callback(self, msg: Float64) -> None:
        #self.get_logger().info(f"Got heading: {msg.data}")
    ...
    def remove_last_points_if_necessary(self, next_point):
        ...
        have looped back and are further away than earlier points, requiring a path correction
    ...
    def calculate_exact_points_from_waypoint(self, waypoint_msg: Waypoint) -> None:
        ...
        This function assumes that 'latitude', 'longitude', 'bbox','image_width', and 'image_height' are 
    ...
    def recalculate_path_from_exact_points(self) -> None:
        ...
        if len(self.grid_points) == 0:
            ...
            return
        path_segments = []
        first = self.get_path(self.current_grid_cell, self.grid_points[0])
        if(first is None):
            return
        path_segments.append(first.path)
        ...
        self.current_combined_path_publisher.publish(combined)
...
def main(args=None):
    ...
    executor.add_node(path_generator)
    ...
    try:
        ...
        path_generator.get_logger().fatal(error_string)
        path_generator.publish_error(error_string)
    finally:
